...
cy.get("[class='onoffswitch2-label']").eq(2).click()// Click on "status" button second time
cy.get("[class='site-checkbox']").eq(1).click()// Click on site select box
cy.get("[class='site-checkbox']").eq(2).click()// Click on site select box
cy.get("[id='bulkDeletePageBtn']").click()// Click on "Bulk Delete" button
cy.get("[type='button']").click()// Click on confirm "Delete/Move" button
    })
})