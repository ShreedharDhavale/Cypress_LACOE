...
cy.get("[class='placeholder:text-[#828A91] placeholder:text-[13px] w-full h-10 p-inputtext p-component']").type(userdata.ITSM_group_name)// Add ITSM group name
cy.get("[class='p-dropdown-trigger']").eq(2).click()// Click on division dropdown button
cy.wait(2000)
cy.get("[class='p-dropdown-item-label']").eq(0).click()// Select division from dropdown
cy.get("[class='p-dropdown-trigger']").eq(3).click()// Click on "Ticket Category" dropdown button
...
cy.get("[class='p-inputtextarea p-inputtext p-component']").eq(0).type(userdata.ITSM_group_description)//Add description
cy.wait(5000)
